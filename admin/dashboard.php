...
<main class="dashboard">
  <div class="card">
    <h2>Muamala: Deposits & Payouts</h2>
    <?php if (!$items): ?>
        ...
        <div class="card" style="margin: 10px 0;">
          <?php if (($p['kind'] ?? 'deposit') === 'deposit'): ?>
            ...
            <form method="POST" action="/admin/post.php" style="display:inline;">
              ...
              <button class="btn btn-primary" name="action" value="approve_deposit">Kubali</button>
              <button class="btn btn-danger" name="action" value="reject_deposit" onclick="return confirm('Kukataa muamala huu?')">Kataa</button>
              <button class="btn" name="action" value="delete_deposit" onclick="return confirm('Futa kabisa muamala huu?')">Futa</button>
            </form>
          <?php else: ?>
            ...
            <form method="POST" action="/admin/post.php" style="display:inline;">
              <input type="hidden" name="csrf" value="<?= htmlspecialchars(csrf_token()) ?>">
              <input type="hidden" name="id" value="<?= htmlspecialchars($p['id'] ?? '') ?>">
              <button class="btn btn-primary" name="action" value="approve_payout">Lipa</button>
              <button class="btn btn-danger" name="action" value="reject_payout" onclick="return confirm('Kataa ombi la payout?')">Kataa</button>
              <button class="btn" name="action" value="delete_payout" onclick="return confirm('Futa ombi hili?')">Futa</button>
            </form>
          <?php endif; ?>
        </div>
      <?php endforeach; ?>
